...
#if 0
{
	...
	for (b = 0; b < ARRAY_SIZE(tegra_gpio_banks); b++) {
		...
		for (p = 0; p < ARRAY_SIZE(bank->oe); p++) {
			...
			printk("bank[%d] : cnf[%2x] : out[%2x] : oe[%2x] : int_enb[%2x] : int_lvl[%2x] \n",p, \
			__raw_readl(GPIO_CNF(gpio)), __raw_readl(GPIO_OUT(gpio)), __raw_readl(GPIO_OE(gpio)), \
			__raw_readl(GPIO_INT_ENB(gpio)), __raw_readl(GPIO_INT_LVL(gpio)));
		}
	}
	local_irq_restore(flags);
}
