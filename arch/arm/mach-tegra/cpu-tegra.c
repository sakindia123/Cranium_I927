...
static bool force_lock = false;
static bool is_cpufreq_locked;
static int cpulock_freq;
static int cpulock_debug_timeout;
static struct hrtimer cpulock_timer;
void tegra_cpu_lock_speed(int rate, int timeout_ms, bool force);
void tegra_cpu_unlock_speed(bool force);
...
{
	if (val)
		tegra_cpu_lock_speed(val, cpulock_debug_timeout, false);
	else {
 ...
 * min_rate is in KHz.
 */
void tegra_cpu_lock_speed(int min_rate, int timeout_ms, bool force)
{
	int idx = 0,found = 0;
	...
	if (force_lock && !force) {
		printk(KERN_DEBUG "%s already force rate set \n", __func__);
		return;
	}
	mutex_lock(&tegra_cpulock_lock);

	printk(KERN_DEBUG "%s: min_rate(%d),timeout(%d) force(%d)\n",
	       __func__, min_rate, timeout_ms, force);
